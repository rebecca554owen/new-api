...
func TestGrantTokenRemainQuotaBatchUpdatesRemainOnly(t *testing.T) {
	...
	if err := DB.First(&Token{}, "id = ? AND remain_quota = ? AND used_quota = ?", token.Id, 200, 80).Error; err != nil {
		t.Fatalf("expected db row to remain unchanged before batch flush: %v", err)
	}
	...
	updatedToken, err := GetTokenById(token.Id)
	if err != nil {
		t.Fatalf("failed to reload token after batch flush: %v", err)
	}
